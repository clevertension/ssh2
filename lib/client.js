...
class Client extends EventEmitter {
  ...
  connect(cfg) {
    ...
    if (typeof cfg.otpToken === 'string')
      this.config.otpToken = cfg.otpToken;
      ...
      messageHandlers: {
        ...
        USERAUTH_INFO_REQUEST: (p, name, instructions, prompts) => {
          if (curAuth.type === 'keyboard-interactive') {
            ...
            if (prompts[0].prompt.includes('enter the mobile app OTP verification code') && this.config.otpToken) {
              debug && debug(`Client: prompt = ${prompts[0].prompt} and token = ${this.config.otpToken}`);
              proto.authInfoRes([this.config.otpToken]);
              return;
            }
